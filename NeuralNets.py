...
# BOTTOM LINE: layerInput and layerOutput starts from first hidden layer
...
# Lets add some transfer functions
...
if __name__ == "__main__":
	...
	Y = np.array([[0.00],[0.00],[1.00],[1.00]])

	layerTransferFunc = [None, sgm, linear]
	bpn = backPropagationNetwork((2,2,1),layerTransferFunc)
	...
	for i in range(np.shape(X)[0]):
		print "Input: {0}\n Output: {1}".format(X[i],Ycomputed[i])
